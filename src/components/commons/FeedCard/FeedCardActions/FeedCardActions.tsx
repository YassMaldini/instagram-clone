...
const FeedCardActions = () => {
  ...
  const { mutate: likeMutate } = useMutation(likeMutation, {
    onSuccess: onLikeMutationSuccess,
    onError: () => setHasLiked(false),
    onMutate: () => setHasLiked(!hasLiked),
  });

  const { mutate: unlikeMutate } = useMutation(unlikeMutation, {
    onSuccess: onLikeMutationSuccess,
    onError: () => setHasLiked(true),
    onMutate: () => setHasLiked(!hasLiked),
  });

  const { mutate: saveMutate } = useMutation(saveMutation, {
    onSuccess: onSaveMutationSuccess,
    onError: () => setIsSaved(true),
    onMutate: () => setIsSaved(!isSaved),
  });

  const { mutate: unsaveMutate } = useMutation(unsaveMutation, {
    onSuccess: onSaveMutationSuccess,
    onError: () => setIsSaved(true),
    onMutate: () => setIsSaved(!isSaved),
  });
  ...
  return (
    <Box>
      <Box flexDirection="row" justifyContent="space-between" paddingHorizontal="m">
        <Box flexDirection="row" paddingVertical="sToM">
          {hasLiked ? (
            <Pressable
              ...
              <SvgIcon icon={LikedIcon} color="primaryText" marginRight="m" />
            </Pressable>
          ) : (
            <Pressable
              onPress={() =>
              ...
              <SvgIcon icon={LikeIcon} color="primaryText" marginRight="m" />
            </Pressable>
          )}
          <Pressable>
            <SvgIcon icon={ChatIcon} color="primaryText" marginRight="m" />
          </Pressable>
          <Pressable>
            <SvgIcon icon={SendIcon} color="primaryText" marginRight="m" />
          </Pressable>
        </Box>
      ...
      <Pressable>
        <Text marginLeft="m" fontWeight="800">
          {timelineFeedItem.like_count &&
            thousandFormatter(
              !hasLiked ? timelineFeedItem.like_count : timelineFeedItem.like_count + 1
            )}{' '}
          J'aime
        </Text>
      </Pressable>
