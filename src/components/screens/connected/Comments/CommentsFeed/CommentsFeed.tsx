        ...
        ListHeaderComponent={
          <>
            {commentsFeed?.caption && <CommentsCaption {...commentsFeed.caption} />}
            {
              ...
              newComments.map(commentItem => <CommentsItem {...{ commentItem }} key={commentItem.pk} />)
            }
          </>
        }
        estimatedItemSize={15}
      />
      <KeyboardAccessoryView style={{ backgroundColor: theme.colors.highlightBackground }} alwaysVisible={true} androidAdjustResize>
        {({ isKeyboardVisible }) => (
          <Box>
            <Box
              flexDirection="row"
