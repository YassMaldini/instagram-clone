...
const ProfileInfos = () => {
  const { userInfos, isCurrentUser, isDiscoverVisible, setDiscoverVisible } = useContext(ProfileScreenContext);
  ...
  return (
    <Box paddingHorizontal="m" marginBottom="s">
      ...
      <Text marginTop="s" fontWeight="800">{full_name}</Text>
      ...
      {Boolean(isCurrentUser) &&
        <Box flexDirection="row" justifyContent="space-between" marginTop="s">
          <Pressable 
            ...
            marginRight="s"
          >
            <Text alignSelf="center" fontWeight="800">Edit profile</Text>
          </Pressable>
          <Pressable 
            backgroundColor="highlightBackground" 
            borderRadius="s"
