...
const FEED_ENDPOINTS = Object.freeze({
  timelineFeed: (data: object) => {
    return api.post<TimelineFeedSuccessResponseData, ErrorResponseData>(
      StaticPostEndpoints.TimelineFeed,
      qs.stringify(data)
    );
  },
  reelsTrayFeed: (data: object) => {
    return api.post<ReelsTrayFeedSuccessResponseData, ErrorResponseData>(
      StaticPostEndpoints.ReelsTrayFeed,
      qs.stringify(data)
    );
  },
